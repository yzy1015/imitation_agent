...
class learning_agent(object):
    def __init__(self, action_shape, observation_shape, env):
        ...
        self.state, self.action =  self.build_net(self.action_shape, self.observation_shape)
        # trajectory and regression 
        self.expert_action = tf.placeholder(tf.float32, [None, self.action_shape])
    ...
    def build_net(self, act_shape, obs_shape):
        ...
        action = tf.layers.dense(inputs = l1, units = act_shape, activation = tf.nn.relu, 
                             kernel_initializer = tf.random_normal_initializer(0.,.1), 
...
def imitation():
    ...
    agent = learning_agent(env.action_space.shape[0], env.observation_space.shape[0], env) 
    for _ in range(80):
        sc = 0
        ...
        while done == False:
            action = pi.act(observation, env) # collect traj and agent current policy
